class Forces
{
  ...
  void calculateDensity(Particle p1, Particle p2) 
  {
    ...
    //p1.setDensity(poly6Const);
    //p2.setDensity(poly6Const);
  ...
  void calculateSurfaceTension(Particle p1, Particle p2) 
  {
    ...
    if ((hSquared > rSquared) && (rSquared!= 0)) 
    {
      ...
      PVector surfaceVecForce = new PVector(surfaceForce * (distance.x / rSquared), surfaceForce * (distance.y / rSquared));
  ...
  {
     float xGravForce = p.gravAcceleration.x * p.getDensity();
     float yGravForce = p.gravAcceleration.y * p.getDensity();
     p.gravForce.set(xGravForce,yGravForce);
     p.accelerate(xGravForce, yGravForce);
  }
